...
Param(
    ...
    Foreach ($server in $Global:servers) {
        $num++
        Write-Host -ForegroundColor Yellow (" $num) " + $Global:servers[$num-1].server)
    }#End Foreach
...
------------------------------------------------------------------------")
    ...
    Write-Host -ForegroundColor Yellow ("`n   3) Domain: All RUNNING Services `t`t`tEnter: 3")
    Write-Host -ForegroundColor Yellow ("`n   4) Domain: All STOPPED Services `t`t`tEnter: 4")
    Write-Host -ForegroundColor Yellow ("`n   ----------")
    Write-Host -ForegroundColor Yellow ("`n   5) Target Machine: ALL Services  `t`t`tEnter: 5")
    Write-Host -ForegroundColor Yellow ("`n   6) Target Machine: All RUNNING Services `t`tEnter: 6")
    Write-Host -ForegroundColor Yellow ("`n   7) Target Machine: All STOPPED Services `t`tEnter: 7")
    Write-Host -ForegroundColor Yellow ("`n   ----------")
...
do
{
    # Get server status to stop constant checks in the program
    # Only check if first round in menu
    ...
    Write-Host("`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n`n") # The screen can leave residual text. 
    Clear-Host
    ...
    switch ($choice) 
    {
        ...
        '5' 
        {
            ...
            do { 
                ...
                If($Global:servers[$choice - 1].active -eq $true) {
                    $machineName = ($Global:servers[$choice - 1].server)
                    Get-Services -machineName $machineName -status "All"
                } else {
                    Write-Host -ForegroundColor Red ("`n**********`n Machine ")
                    ($Global:servers[$choice - 1].server)
                    Write-Host -ForegroundColor Red (" Is Not Active `n**********`n")
                }#End If/else
